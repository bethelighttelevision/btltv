...
export default function NewTeamMemberPage() {
  ...
  const [form, setForm] = useState({ name: "", designation: "", photo: "", bio: "", displayOrder: 0 });

  const save = async () => {
    if (!form.name) { toast.error("Name is required"); return; }
    setSaving(true);
    const res = await fetch("/api/admin/team", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
    if (res.ok) { toast.success("Team member created"); router.push("/admin/team"); }
  ...
  return (
    <div>
      ...
      <div className="max-w-lg space-y-4">
        ...
        <div>
          <label className="text-[10px] text-gray-500 uppercase tracking-wider font-semibold">Designation / Role</label>
        ...
        <div className="flex items-center gap-2 pt-2">
          <button onClick={save} disabled={saving}
            ...
            <Save className="h-3.5 w-3.5" /> {saving ? "Saving..." : "Save Member"}
          </button>
