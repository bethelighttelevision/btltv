...
const CATEGORIES = ["Talk Show", "Devotional", "Documentary", "Drama", "Social Issues", "Health", "Education", "Bible School", "Kids", "News"];

export default function NewShowPage() {
  const router = useRouter();
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ title: "", titleUrdu: "", slug: "", category: "Talk Show", playlistId: "", thumbnail: "", description: "", isActive: true, isFeatured: false });
  ...
  const save = async () => {
    ...
    if (res.ok) { toast.success("Show created"); router.push("/admin/shows"); }
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-xl font-bold text-white">Add New Show</h1>
      </div>
      <div className="max-w-2xl space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="md:col-span-2">
            ...
            <input value={form.playlistId} onChange={(e) => setForm({ ...form, playlistId: e.target.value })} placeholder="PLC0Rch0KTiEL1XcXiXO76FeMysmOQda-v"
              className="w-full bg-black/50 border border-white/[0.08] text-white text-sm rounded-lg px-3 h-10 mt-1 focus:outline-none focus:border-btl-red/50" />
          </div>
          <div className="md:col-span-2">
        ...
        <div className="flex items-center gap-2 pt-2">
          <button onClick={save} disabled={saving}
            ...
            <Save className="h-3.5 w-3.5" /> {saving ? "Saving..." : "Save Show"}
          </button>
