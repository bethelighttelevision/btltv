...
export default function AboutPageEditor() {
  ...
  const [form, setForm] = useState({ pageTitle: "", heroBanner: "", introText: "", mission: "", vision: "", content: "", ctaText: "", ctaLink: "", metaTitle: "", metaDescription: "", ogImage: "" });
  ...
  const save = async () => {
    ...
    if (res.ok) toast.success("About page saved"); else toast.error("Failed to save");
    setSaving(false);
  };

  if (loading) return <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-btl-red" /></div>;

  return (<div><h1 className="text-xl font-bold text-white mb-6">About Page Editor</h1>
    <div className="max-w-lg space-y-4">
      <Field label="Page Title" v={form.pageTitle} onChange={(v) => setForm({ ...form, pageTitle: v })} />
      <TA label="Intro Text" v={form.introText} onChange={(v) => setForm({ ...form, introText: v })} />
      <TA label="Mission" v={form.mission} onChange={(v) => setForm({ ...form, mission: v })} />
      <TA label="Vision" v={form.vision} onChange={(v) => setForm({ ...form, vision: v })} />
      <TA label="Content" v={form.content} onChange={(v) => setForm({ ...form, content: v })} />
      <Field label="CTA Text" v={form.ctaText} onChange={(v) => setForm({ ...form, ctaText: v })} />
      <Field label="CTA Link" v={form.ctaLink} onChange={(v) => setForm({ ...form, ctaLink: v })} />
      <Divider />
      <Field label="Meta Title" v={form.metaTitle} onChange={(v) => setForm({ ...form, metaTitle: v })} />
...
function Divider() { return <div className="pt-4 border-t border-white/[0.04]"><h3 className="text-xs text-gray-400 font-semibold mb-3">SEO</h3></div>; }
