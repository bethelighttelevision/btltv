...
export default function KidsPageEditor() {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ pageTitle: "", heroBanner: "", introText: "", content: "", metaTitle: "", metaDescription: "", ogImage: "" });
  ...
  const save = async () => {
    ...
    if (res.ok) toast.success("Kids page saved"); else toast.error("Failed to save");
    setSaving(false);
  };

  if (loading) return <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-btl-red" /></div>;

  return (<div><h1 className="text-xl font-bold text-white mb-6">Kids Page Editor</h1>
