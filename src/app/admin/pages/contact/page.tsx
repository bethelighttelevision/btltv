...
export default function ContactPageEditor() {
  ...
  const [form, setForm] = useState({ pageTitle: "", heroBanner: "", email: "", phone: "", address: "", mapEmbed: "", introText: "", metaTitle: "", metaDescription: "", ogImage: "" });
  ...
  const save = async () => {
    ...
    if (res.ok) toast.success("Contact page saved"); else toast.error("Failed to save");
    setSaving(false);
  };

  if (loading) return <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-btl-red" /></div>;

  return (<div><h1 className="text-xl font-bold text-white mb-6">Contact Page Editor</h1>
    <div className="max-w-lg space-y-4">
      ...
      <F label="Phone" v={form.phone} onChange={(v) => setForm({ ...form, phone: v })} />
      <TA label="Address" v={form.address} onChange={(v) => setForm({ ...form, address: v })} />
      <TA label="Map Embed HTML" v={form.mapEmbed} onChange={(v) => setForm({ ...form, mapEmbed: v })} />
