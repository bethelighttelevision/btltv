...
export default function ReportsContent() {
  const renderReportCard = (report: { title: string; year: string; file: string }, subtitle: string) => (
    <motion.div key={report.title} whileHover={{ scale: 1.01 }}>
      <a href={report.file} target="_blank" rel="noopener noreferrer" className="block">
        <Card className="bg-btl-card border-btl-card-border hover:border-btl-red/30 transition-all cursor-pointer">
          <CardContent className="p-4 flex items-center gap-4">
            <div className="h-12 w-12 rounded-lg bg-btl-red/10 flex items-center justify-center shrink-0"><FileText className="h-6 w-6 text-btl-red" /></div>
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-foreground">{report.title}</h3>
              <p className="text-xs text-muted-foreground mt-0.5">{subtitle}</p>
            </div>
            <Badge className="bg-btl-dark text-muted-foreground text-xs shrink-0">{report.year}</Badge>
            <Button variant="ghost" size="sm" className="text-btl-red hover:text-btl-red-dark shrink-0"><ExternalLink className="h-4 w-4 mr-1" /><span className="text-xs">View</span></Button>
          </CardContent>
        </Card>
      </a>
    </motion.div>
    ...
    <div className="min-h-screen px-4 md:px-6 py-6 max-w-4xl mx-auto">
      ...
      <Card className="bg-btl-card border-btl-card-border mb-8 overflow-hidden">
        <div className="bg-gradient-to-r from-btl-red/10 to-transparent p-4 md:p-6 border-b border-btl-card-border">
          <div className="flex items-center gap-3"><Shield className="h-5 w-5 text-btl-red" /><h2 className="text-lg font-bold text-foreground">ANBI Registered Foundation</h2></div>
        </div>
        <CardContent className="p-6">
          <div className="flex flex-col sm:flex-row items-center gap-6">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed">
              ...
              As an ANBI-certified organization, we are committed to transparency and publish our annual reports and financial statements for public review.
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-4">
          <div className="h-10 w-10 rounded-lg bg-btl-red/10 flex items-center justify-center shrink-0"><FileText className="h-5 w-5 text-btl-red" /></div>
          <div>
        ...
        <div className="space-y-3">{ANNUAL_REPORTS.map((report) => renderReportCard(report, "Annual Financial Statement — Stichting BTL TV"))}</div>
      </div>

      <div className="mb-10">
        <div className="flex items-center gap-3 mb-4">
          <div className="h-10 w-10 rounded-lg bg-btl-red/10 flex items-center justify-center shrink-0"><Newspaper className="h-5 w-5 text-btl-red" /></div>
          <div>
        ...
        <div className="space-y-3">{OTHER_REPORTS.map((report) => renderReportCard(report, "Official Document — Stichting BTL TV"))}</div>
      </div>

      <div className="text-center mt-8">
        <p className="text-muted-foreground text-sm">
          For more information about our reports, please contact us at{" "}
          <a href="mailto:info@btl-tv.com" className="text-btl-red hover:underline">info@btl-tv.com</a>
        </p>
      </div>
    </div>
  );
