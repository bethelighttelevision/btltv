      ...
      <div className="max-w-6xl mx-auto space-y-12">
        {TEAM_GROUPS.map((group) => (
          <div key={group.title}>
            ...
            <div className={`grid gap-4 md:gap-5 ${group.members.length <= 2 ? "grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4" : group.members.length <= 4 ? "grid-cols-2 sm:grid-cols-3 md:grid-cols-4" : "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5"}`}>
              {group.members.map((member) => (
                <motion.div key={member.name} whileHover={{ y: -4, scale: 1.02 }} className="cursor-default">
                  <div className="overflow-hidden rounded-xl bg-btl-card border border-btl-card-border shadow-lg shadow-black/5 hover:shadow-xl hover:shadow-btl-red/5 hover:border-btl-red/30 transition-all duration-300">
                    <div className="relative aspect-[3/4] overflow-hidden">
                      <img src={member.image} alt={member.name} width={500} height={500} loading="lazy" className="w-full h-full object-cover object-top"
                        onError={(e) => { (e.target as HTMLImageElement).src = "/images/logo/btl-logo.webp"; }} />
